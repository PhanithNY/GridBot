...
class SimpleGrid(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Entry logic: Only enter when price touches specific grid levels
        """
        ...
        for i in range(-num_levels, 0):  # Only negative levels for buy entries
            ...
            if grid_level_col in dataframe.columns:
                # Entry condition: price touches the grid level
                # We consider it "touching" if the low of the candle is at or below the grid level
                # and the high of the candle is above the grid level (price crossed through it)
                dataframe.loc[
                    (
                        (dataframe['low'] <= dataframe[grid_level_col]) &  # Price touched or went below the level
                        (dataframe['high'] > dataframe[grid_level_col])   # Price also went above the level (crossed through)
                    ),
                    'enter_long'
                ] = 1
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        2. Take profit: Exit if price goes above the grid level where we entered
        """
        ...
        # 2. Take Profit: Exit if price goes above the grid level where we entered
        # We need to track which grid level we entered at and exit when price goes above the corresponding positive level
        # For now, we'll implement a simplified version that exits when price goes above any positive grid level
        # This can be enhanced later to track the specific entry level
        ...
        for i in range(1, num_levels + 1):  # Positive levels for take profit
            ...
            if grid_level_col in dataframe.columns:
                dataframe.loc[
                    (dataframe['close'] > dataframe[grid_level_col]),  # Price above take profit level
                    'exit_long'
                ] = 1
